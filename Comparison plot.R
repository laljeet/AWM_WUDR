...
plot2 <- plot_dat
plot2$Method1_Unreported <- log(plot2$Method1_Unreported)
plot2$Initial_Method1_Unreported <- log(plot2$Initial_Method1_Unreported)
...
max <- max(c(max(plot_dat$Initial_Method1_Unreported), na.rm = TRUE))

min <- min(c(min(plot_dat$Initial_Method1_Unreported), na.rm = TRUE))
...
p1 <- ggplot(plot_dat, aes(x=Initial_Method1_Unreported, y=Method1_Unreported))+
  geom_point()+
  # geom_abline()+
  # geom_smooth(method=lm)+
  labs(title = "SF Unreported withdrawals (MG)",
       x="Percenatge Area proportion (MG)", y = " Deficit Irrigtaion (MG)")+
...
ggsave("F:/My Drive/WUDR/WUDR Manuscript/Plots/Scatter_old_new_Method.png", plot = p1, width = 12, height = 8, units = "in")
