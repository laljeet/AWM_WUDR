...
##################################################################
  ...
  plot_dat[4:6] <- plot_dat[4:6]*3785
  sc = 2
  # p1 <- ggplot(plot_dat, aes(x = Year, y = Unreported_Deficit_Irr_based/sc)) +
     p1 <- ggplot(plot_dat, aes(x = Year, y = Unreported_Deficit_Irr_based /10^4 )) +
  geom_line(aes(color = "LF unreported "), lwd=1) +
  geom_line(aes(y = IRR_DEQ_withdrawals/10^4, color = "VDEQ reported "), lwd=1)+
  geom_line(aes(y = Small_Farm_Unreported/10^4, color = "SF Unreported "), lwd=1)+
  ...
  # scale_y_continuous(limits = c(0,2000), breaks = seq(0,2000, by = 500))+
    scale_y_continuous(limits = c(0,round_any(775054,100000)/10^4 ), breaks = seq(0,round_any(775054,100000/10^4), by = 200000/10^4))+
                       # ,sec.axis = sec_axis(~.*sc, name="LF unreported (MG)")) +
    
  labs(title= "Shenandoah" , x = "Year", y = bquote('Withdrawals (' ~ 10^'4' ~ 'x m' ^'3'~')'), color = "") +
 ...
 ggsave(paste0("F:/My Drive/WUDR/WUDR Manuscript/Plots/shenancm.png"), plot = p1, width = 9.5, height = 6, units = "in")
...
#   
  plot_dat <- North[,c(1:3,8,10,11)]
   plot_dat[4:6] <- plot_dat[4:6]*3785
  sc = 20
  p2 <- ggplot(plot_dat, aes(x = Year, y = Unreported_Deficit_Irr_based /10^4)) +
  geom_line(aes(color = "LF unreported "), lwd=1) +
  geom_line(aes(y = IRR_DEQ_withdrawals/10^4 , color = "VDEQ reported "),lwd=1)+
  geom_line(aes(y = Small_Farm_Unreported/10^4*sc, color = "SF Unreported "), lwd=1)+
   scale_x_continuous(limits = c(2002, 2017),  breaks = seq(2002, 2017, by = 1))+
  # scale_y_continuous(limits = c(0,2000), breaks = seq(0,2000, by = 500))+
    scale_y_continuous(limits = c(0,8000000/10^4), breaks = seq(0,8000000/10^4, by = 2000000/10^4), 
                       sec.axis = sec_axis(~./sc, name= bquote('SF Unreported (' ~ 10^'4' ~ 'x m' ^'3'~')'))) +
    
  labs(title= "Northampton" , x = "Year", y = bquote('LF unreported / VDEQ reported ('~ 10^'4' ~ 'x m' ^'3'~')') , color = "") +
  scale_color_manual(values = c("#66c2a5", "#fc8d62", "#8da0cb"))
    p2<- p2 + theme_bw()
  ...
  p2
plotly::ggplotly(p2)
 ggsave(paste0("F:/My Drive/WUDR/WUDR Manuscript/Plots/NORTHAMPTONcu.png"), plot = p2, width = 9.5, height = 6, units = "in")

 plot_dat <- Pow[,c(1:3,8,10,11)]
 plot_dat[4:6] <- plot_dat[4:6]*3785
 
 p3 <- ggplot(plot_dat, aes(x = Year, y = Unreported_Deficit_Irr_based /10^4)) +
  geom_line(aes(color = "LF unreported"), lwd=1) +
  geom_line(aes(y = IRR_DEQ_withdrawals /10^4, color = "VDEQ reported "), lwd=1)+
  geom_line(aes(y = Small_Farm_Unreported /10^4, color = "SF Unreported"), lwd=1)+
   scale_x_continuous(limits = c(2002, 2017),  breaks = seq(2002, 2017, by = 1))+
  # scale_y_continuous(limits = c(0,2000), breaks = seq(0,2000, by = 500))+
    scale_y_continuous(limits = c(0,85000 /10^4), breaks = seq(0,85000/10^4, by = 20000/10^4)) +
    
  labs(title= "Powhatan" , x = "Year", y = bquote('Withdrawals (' ~ 10^'4' ~ 'x m' ^'3'~')'), color = "") +
  scale_color_manual(values = c("#66c2a5", "#fc8d62", "#8da0cb"))
    p3<- p3 + theme_bw()
 ...
 ggsave(paste0("F:/My Drive/WUDR/WUDR Manuscript/Plots/Powhatancu.png"), plot = p3, width = 9.5, height = 6, units = "in")
  ...
  ggsave("F:/My Drive/WUDR/WUDR Manuscript/Plots/secondaryAxis_counties2.png", plot = p_both, width = 8, height =12, units = "in")
