...
SF_plot_fn <- function(Year){
  ...
  Unreported_Under_TH$Method1_Unreported <- Unreported_Under_TH$Method1_Unreported *3785 / 1000 # Use 10 ^ 3 in breaks
  ...
  p4<-tm_shape(plotdat)+
    tm_polygons("Method1_Unreported", title = expression(paste("Unreported Volume ", "(", 10^{3}, " x ", m^3, ")")),
                breaks = c(0,18,37,74,148,296,383),
                # n=5,style="jenks",
...
p_both <-  tmap_arrange(plot_2002,plot_2007,plot_2012,plot_2017,nrow=2, ncol=2)
 ...
 tmap_save(p_both, paste0("F:/My Drive/WUDR/WUDR Manuscript/Plots/", "_SF_ALL_SI.png"),  width = 10, height = 6, units = 'in')
