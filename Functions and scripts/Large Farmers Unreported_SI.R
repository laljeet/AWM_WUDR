...
for (i in 1:length(DEQ_Tot_area_Dat)) {
 ...
 Large_DEQ_Tot_area[[i]]$Large_Farm_unreported <- Large_DEQ_Tot_area[[i]]$Large_Farm_unreported*3785 / 1000 # Use 10 ^ 3 in breaks
}
...
fn_plot_Tot_area <- function(Year){
  ...
  p2<-tm_shape(plot_Dat)+
    tm_polygons("Large_Farm_unreported", title = expression(paste("Unreported Volume ", "(", 10^{3}, " x ", m^3, ")")),
                breaks = c(0,37,186,371,742,1860,Inf),
                # n=5,style="jenks",
                textNA = "-ve coefficient value/No Census data",
                id="NAMELSAD") +
    # tm_text("NAME", size = 0.3)+
   ...
   tmap_save(p2, paste0("F:/My Drive/WUDR/WUDR Manuscript/Plots/",Year, "_LF_VOL_DEQ_Missing_counties_SI.png"),  width = 8.5, height = 5, units = 'in')
...
legend.map <- tm_shape(plot_Dat)+
  tm_polygons("Large_Farm_unreported", title = expression(paste("Unreported Volume ( ", m^{3}, ")")),
               breaks = c(0,37000,186000,371000,742000,1860000,Inf),
              # n=5,style="jenks",
              textNA = "-ve coefficient value/No Census data")+
  tm_layout(legend.only = TRUE,design.mode=TRUE, scale=2, asp=0)

tmap_save(legend.map, paste0("F:/My Drive/WUDR/WUDR Manuscript/Plots/",Year, "Legend_SI.png"),  width = 8.5, height = 5, units = 'in')
# library(grid)
# grid.newpage()
...
# p_both <-  tmap_arrange(p1,p2,p3,p4, nrow=2, ncol = 2,main = quote("Combined Maps"), legend.position = "bottom")
#   p_both  

 tmap_save(p_both, paste0("F:/My Drive/WUDR/WUDR Manuscript/Plots/", "_LF_ALL_SI.png"),  width = 10, height = 6, units = 'in')
