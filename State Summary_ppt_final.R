...
DEQ_Irr<- Irri_deq_county %>%
  filter(YEAR <2018) %>% 
  group_by(YEAR) %>% 
  summarise(`VDEQ Irrigation` = sum(Facility_withdrawal_mg))
...
plot_dat$`Unreported Wth` <- plot_dat$`Unreported Wth`*3.068
p1 <- ggplot(plot_dat, aes(x=YEAR, y=`Unreported Wth`, group = Type ))+
  geom_line(aes(color=Type))+
  geom_point(aes(color=Type))+
  labs(title= " " ,
       x="Year", y = " Unreported Withdrawals \n(Acre-feet)")+
...
ggsave(paste0("F:/My Drive/WUDR/WUDR Manuscript/Plots/","State_Summary_unreported_WTHAF1.png"), plot = p1, width = 9.5, height = 6, units = "in")
