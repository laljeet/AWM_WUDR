...
##################################################################
  ...
  plot_dat[4:6] <- plot_dat[4:6]*3785/1000
  ...
  labs(title= "Shenandoah" , x = "Year", y = expression(paste("Withdrawals ", "(", 10^{3}, " x ", m^3, ")")), color = "") +
 ...
 ggsave(paste0("F:/My Drive/WUDR/WUDR Manuscript/Plots/shenanSI.png"), plot = p1, width = 9.5, height = 6, units = "in")
...
#   
  ...
  # scale_y_continuous(limits = c(0,2000), breaks = seq(0,2000, by = 500))+
    scale_y_continuous(limits = c(0,8000), breaks = seq(0,8000, by = 1000), 
                       sec.axis = sec_axis(~./sc, name=expression(paste("SF Unreported ", "(", 10^{3}, " x ", m^3, ")")),)) +
    
  labs(title= "Northampton" , x = "Year", y = expression(paste("LF Unreported ", "(", 10^{3}, " x ", m^3, ")")),  color = "") +
  scale_color_manual(values = c("#66c2a5", "#fc8d62", "#8da0cb"))
    p2<- p2 + theme_bw()
  ...
  # plotly::ggplotly(p2)
 ggsave(paste0("F:/My Drive/WUDR/WUDR Manuscript/Plots/NORTHAMPTONSI.png"), plot = p2, width = 9.5, height = 6, units = "in")
 ...
 p3 <- ggplot(plot_dat, aes(x = Year, y = Unreported_Deficit_Irr_based )) +
  ...
  labs(title= "Powhatan" , x = "Year", y = expression(paste("Withdrawals ", "(", 10^{3}, " x ", m^3, ")")), color = "") +
  scale_color_manual(values = c("#66c2a5", "#fc8d62", "#8da0cb"))
    p3<- p3 + theme_bw()
 ...
 ggsave(paste0("F:/My Drive/WUDR/WUDR Manuscript/Plots/PowhatanSI.png"), plot = p3, width = 9.5, height = 6, units = "in")
  ...
  ggsave("F:/My Drive/WUDR/WUDR Manuscript/Plots/secondaryAxis_countiesSI.png", plot = p_both, width = 8, height =12, units = "in")
