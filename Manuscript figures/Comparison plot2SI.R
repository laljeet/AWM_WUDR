...
plot_dat[3:4] <- plot_dat[3:4]*3785/1000
max <- max(c(max(plot_dat$DEQ_reported), na.rm = TRUE))
...
p1 <- ggplot(plot_dat, aes(x=Irrigation_withdrawals, y=DEQ_reported, color= status ))+
  ...
  labs(title = "",
       x=expression(paste("Irrigation withdrawals ", "(", 10^{3}, " x ", m^3, ")")),
       y =expression(paste("VDEQ Reported ", "(", 10^{3}, " x ", m^3, ")")))+
...
ggsave("F:/My Drive/WUDR/WUDR Manuscript/Plots/Scatter_colorSI1.png", plot = p1, width =12, height =6, units = "in")
