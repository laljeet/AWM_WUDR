...
plot_dat[3:4] <- plot_dat[3:4]*3785
max <- max(c(max(plot_dat$DEQ_reported), na.rm = TRUE))
...
p1 <- ggplot(plot_dat, aes(x=Irrigation_withdrawals, y=DEQ_reported, color= status ))+
  ...
  labs(title = "",
       x= bquote('Irrigation Withdrawals ('~ 'm' ^'3'~')'), y = bquote('DEQ Reported ('~ 'm' ^'3'~')'))+
...
ggsave("F:/My Drive/WUDR/WUDR Manuscript/Plots/Scatter_color2cu.png", plot = p1, width =8, height =6, units = "in")
